...
class GameSurface(ScreenHandle):
    ...
    def draw_map(self):
        sprite_size, w, h = self.game_engine.sprite_size, self.get_width(), self.get_height()

        x_threshold = int(0.6*w/sprite_size)
        y_threshold = int(0.6*h/sprite_size)
        
        if self.game_engine.hero.position[0] > 36:
            min_x = 36 - x_threshold
        elif self.game_engine.hero.position[0] > x_threshold:
            min_x = self.game_engine.hero.position[0] - x_threshold
        else:
            min_x = 0

        if self.game_engine.hero.position[1] > 37:
            min_y = 37 - y_threshold
        elif self.game_engine.hero.position[1] > y_threshold:
            min_y = self.game_engine.hero.position[1] - y_threshold
        else:
            min_y = 0
...
class MiniMap(ScreenHandle):
    ...
    def draw(self, canvas):
        ...
        hero_sprite = pygame.Surface((4,4))
        hero_sprite.fill(colors["white"])

        self.blit(hero_sprite, (self.engine.hero.position[0]*4,self.engine.hero.position[1]*4))
