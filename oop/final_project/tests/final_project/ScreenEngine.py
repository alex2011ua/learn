...
class GameSurface(ScreenHandle):
    ...
    def draw_hero(self):
        self.game_engine.hero.draw(self, self.min_coord(),
                                   self.game_engine.sprite_size)
...
class ProgressBar(ScreenHandle):
    ...
    def draw(self, canvas):
        ...
        if self.game_engine.hero.hp <= self.game_engine.hero.max_hp:
            max_xp_rectangle = 200 * self.game_engine.hero.hp / \
                               self.game_engine.hero.max_hp
        else:
            max_xp_rectangle = 200

        pygame.draw.rect(self, colors["red"], (55, 36, max_xp_rectangle, 30))
        pygame.draw.rect(self, colors["green"],
                         (55, 76, 200 * self.game_engine.hero.exp /
                          (100 * (2**(self.game_engine.hero.level - 1))), 30))

        font = pygame.font.SysFont("gothici", 20)
        self.blit(font.render(f'Hero at {self.game_engine.hero.position}',
                              True, colors["black"]), (250, 3))

        self.blit(font.render(f'{self.game_engine.level} floor',
                              True, colors["black"]), (10, 3))

        self.blit(font.render(f'HP', True, colors["black"]), (4, 40))
        self.blit(font.render(f'Exp', True, colors["black"]), (4, 80))

        hp_position = 155 - len(str(self.game_engine.hero.hp) +
                                str(self.game_engine.hero.max_hp)) * 10
        exp_position = 155 - len(str(self.game_engine.hero.exp) +
                                 str(100*(2**(self.game_engine.hero.level-1)))) * 9

        self.blit(font.render(f'{self.game_engine.hero.hp}/'
                              f'{self.game_engine.hero.max_hp}', True,
                              colors["black"]), (hp_position, 40))
        self.blit(font.render(f'{self.game_engine.hero.exp}/'
                              f'{(100*(2**(self.game_engine.hero.level-1)))}',
                              True, colors["black"]), (exp_position, 80))
        ...
        self.blit(font.render(f'{self.game_engine.hero.level}',
                              True, colors["black"]), (345, 40))
        gold_position = 345 - (len(str(self.game_engine.hero.gold)) - 1) * 5
        self.blit(font.render(f'{self.game_engine.hero.gold}',
                              True, colors["black"]), (gold_position, 80))
        ...
        self.blit(font.render(f'{self.game_engine.hero.stats["strength"]}',
                              True, colors["black"]), (460, 40))
        self.blit(font.render(f'{self.game_engine.hero.stats["luck"]}',
                              True, colors["black"]), (460, 80))

        self.blit(font.render(f'Score', True, colors["black"]), (525, 40))
        self.blit(font.render(f'{self.game_engine.score:.2f}',
                              True, colors["black"]), (515, 80))
...
class InfoWindow(ScreenHandle):
    ...
    def draw(self, canvas):
        ...
        font = pygame.font.SysFont("gothice", 14)
